# coded in Python 3.6
# beta version finalized 30 Apr 2019
...
# The version is set for Fig. 4 in T86.
...
# fig. 3, so the problem is probably associated with how tropopause height
# is calculated or some related calculation.
...
#theta_bot_final += del_theta_s/(1.0 + (R/R0)**2)
theta_bot_final += del_theta_s*(np.cos(np.pi*R/(4*R0)))**2
theta_bot_final[np.where(R >= 2*R0)] = 0.0
theta_trop_final += ht
...
ir_start = 0
ir_end = r_big.size
ir_start = np.argmin(np.abs(r_big + 2))
ir_end = np.argmin(np.abs(r_big - 2)) + 1
# ***** 5. Plot **********************
...
plt.clabel(cw, vlevs_lab, inline=1, fontsize=6, fmt='%2.0f',
           inline_spacing=8)
plt.xticks(np.array([-6000, -4000, -2000, 0, 2000, 4000, 6000]), fontsize=8)
ax.set_xticklabels(['6000 km', '', '2000 km', 0, '2000 km', '', 
                    '6000 km'])
ax.yaxis.tick_right()
...
plt.title('Wind Speed and Potential Temperature', fontsize=8, 
          fontweight='normal')
plt.savefig('thorpe_1986_fig_4a_calc.png', bbox_inches='tight', dpi=300)


fig, ax = plt.subplots(1, 1, figsize=(5.5, 2))

clevs = np.arange(-50, 10, 10)
clevs_lab = clevs

vlevs = np.array([0.2, 0.5])
vlevs_lab = vlevs
...
cw = plt.contour(r_big_dim[ir_start:ir_end], Z_dim_w, 
                 zof_big_w[:, ir_start:ir_end]-1, vlevs, colors='black', 
...
plt.title('Height Anomalies and Zeta/f', fontsize=8, 
          fontweight='normal')
plt.savefig('thorpe_1986_fig_4b_calc.png', bbox_inches='tight', dpi=300)
