...
# define pot temp at bottom, tropopause, and top
...
theta_trop_final = (1/2)*A*(np.cos(np.pi*R/R0) + 1) + ht
theta_trop_final[np.where(R>R0)] = ht

theta_top_final = np.zeros_like(theta_bot_final)
theta_top_final += ht*(qt - qs) + qs
...
ir_start = np.argmin(np.abs(r_big + 1.5))
ir_end = np.argmin(np.abs(r_big - 1.5)) + 1
...
vlevs = np.array([3, 6, 9, 12, 15, 18, 21])
...
plt.clabel(cw, vlevs_lab, inline=1, fontsize=6, fmt='%2.0f',
           inline_spacing=8)
plt.xticks(np.array([-5001, -3334, -1667, 0, 1667, 3334, 5001]), fontsize=8)
ax.set_xticklabels(['','', '', 0, '', '3334 km'])
ax.yaxis.tick_right()
...
plt.title('Wind Speed and Potential Temperature', fontsize=8, 
          fontweight='normal')
plt.savefig('thorpe_HMR_calc.png', bbox_inches='tight', dpi=300)
...
plt.title('Height Anomalies and Zeta/f', fontsize=8, 
          fontweight='normal')
plt.savefig('thorpe_HMR_calc2.png', bbox_inches='tight', dpi=300)
